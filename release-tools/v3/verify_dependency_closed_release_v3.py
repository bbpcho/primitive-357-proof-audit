...
INDEX = RELEASE / "inputs/DEPENDENCY_CLOSURE_INDEX_V3.json"
BASE_INDEX = RELEASE / "inputs/BASE_EVIDENCE_MANIFEST_INDEX_V1.json"
ROOT_MANIFEST = RELEASE / "manifests/PRIMITIVE_357_DEPENDENCY_CLOSED_RELEASE_V3_SHA256SUMS.txt"
PORTABLE = RELEASE / "scripts/portable_python.py"
SECTOR_INDEX = RELEASE / "inputs/SECTOR_REPLAY_INDEX_V3.json"
FOUNDATION = RELEASE / "scripts/run_foundational_replays_v3.py"
...
def verify_repository() -> tuple[int, int]:
    ...
    for entry in closure["repository_files"]:
        target = REPOSITORY / safe_relative(entry["path"])
        if not target.is_file() or target.is_symlink() or sha256(target) != entry["sha256"]:
            raise AssertionError(("repository file mismatch", entry["path"]))
        expected.add(target.resolve())
...
def verify_sector_index() -> None:
    ...
    assert index["schema"] == "primitive_357_sector_replay_index_v3"
...
def verify_section7_kummer_repair() -> None:
    root = REPOSITORY / "results/2026-09-13_section7_basis_kummer_repair_v1"
    crosscheck = root / "section7_kummer_crosscheck.py"
    search = root / "independent_basis_kummer.py"
    expected_json = root / "independent_basis_kummer.json"
    completed = subprocess.run(
        [sys.executable, "-B", str(crosscheck)],
        cwd=root,
        env=clean_env(),
        text=True,
        stdout=subprocess.PIPE,
    ...
    if completed.returncode or "INDEPENDENT_CROSSCHECK=PASS_RANK4_WITNESS" not in completed.stdout:
        raise AssertionError(("Section 7 Kummer crosscheck", completed.stdout))
    with tempfile.TemporaryDirectory(prefix="primitive357-kummer-") as temporary:
        temp = Path(temporary)
        copied_script = temp / search.name
        copied_json = temp / expected_json.name
        shutil.copy2(search, copied_script)
        shutil.copy2(expected_json, copied_json)
        completed = subprocess.run(
            [sys.executable, "-B", str(copied_script)],
            cwd=temp,
            env=clean_env(),
            text=True,
            stdout=subprocess.PIPE,
        ...
        if completed.returncode:
            raise AssertionError(("Section 7 Kummer search", completed.stdout))
        generated = json.loads(copied_json.read_text(encoding="utf-8"))
        expected = json.loads(expected_json.read_text(encoding="utf-8"))
        if generated != expected or generated.get("status") != "RANK4_WITNESS":
            raise AssertionError("Section 7 Kummer search result mismatch")
    print("SECTION7_KUMMER_REPAIR=PASS_INDEPENDENT_RANK4_MOD2_WITNESS")


def run_foundational_replays() -> None:
    completed = subprocess.run(
        [sys.executable, "-B", str(FOUNDATION)],
        cwd=REPOSITORY,
        env=clean_env(),
        text=True,
        stdout=subprocess.PIPE,
    ...
    if completed.returncode or marker not in completed.stdout:
        raise AssertionError(("foundational replay", completed.returncode, marker))
...
def main() -> None:
    ...
    parser.add_argument("--run-foundational-replays", action="store_true")
    ...
    verify_sector_index()
    verify_section7_kummer_repair()
    if args.run_sector_replays:
        run_sector_replays()
    if args.run_foundational_replays:
        run_foundational_replays()
    print(
        "PRIMITIVE_357_DEPENDENCY_CLOSED_RELEASE_V3=PASS "
        f"REPOSITORY_FILES={files} HISTORICAL_MANIFEST_ROWS={rows} "
        f"UPSTREAM_FILES={upstream_files} "
        f"SECTOR_REPLAYS={int(args.run_sector_replays)} "
        f"FOUNDATIONAL_REPLAYS={int(args.run_foundational_replays)} "
        f"PRESEAL={int(args.preseal)}"
    )
